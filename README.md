...
AI Media Tools 是一个本地优先的 AI 多媒体素材整理 MVP，用来给本地图片、视频、音频建立可搜索的素材索引。默认行为是不移动、不重命名、不修改原始素材，只读取文件元信息并把索引、任务状态、AI 生成内容写入本地 `.data` 目录。
...
- 当前默认 `mock` provider 不上传任何素材。
- 后续接入云端 provider 时，应只上传完成分析所需的最小帧、音频片段或必要元数据。
